...
sigSocket.addEventListener('message', (e) => {console.log(e.data); offerUnvalidated=e.data; gotOffer()})
let otherSocket;
...
let answerReady = (x) => {console.dir(x);}
...
async function gotOffer(){
	...
	console.log(oU)
	let hashValidated = await hashAuthenticatedMessage.verifyAndReturn(oU.Message, conf.PeerPSK, oU.MAC);
...
pc.setRemoteDescription(new wrtc.RTCSessionDescription(JSON.parse(timeValidated)));
//pc.addEventListener('negotiationneeded', pc_negotiation_needed);
//pc.setRemoteDescripotion
let answer = await pc.createAnswer();
pc.setLocalDescription(answer);
if (selftest) setTimeout(() => console.log("Gathered so far: " + JSON.stringify(pc.localDescription)), 8000)
