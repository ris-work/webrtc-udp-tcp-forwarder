...
use webrtc_cc::api::interceptor_registry::register_default_interceptors;
use webrtc_cc::api::media_engine::MediaEngine;
use webrtc_cc::api::setting_engine::SettingEngine;
use webrtc_cc::api::APIBuilder;
use webrtc_cc::data_channel::data_channel_init::RTCDataChannelInit;
use webrtc_cc::data_channel::data_channel_message::DataChannelMessage;
use webrtc_cc::data_channel::RTCDataChannel;
use webrtc_cc::ice::udp_mux::{UDPMuxDefault, UDPMuxParams};
use webrtc_cc::ice::udp_network::UDPNetwork;
use webrtc_cc::ice_transport::ice_server::RTCIceServer;
use webrtc_cc::interceptor::registry::Registry;
use webrtc_cc::peer_connection::configuration::RTCConfiguration;
...
lazy_static! {
    ...
    static ref SocketSendQueue: ArrayQueue<AlignedMessage> =
        ArrayQueue::<AlignedMessage>::new(10);
}
...
) -> (Arc<RTCDataChannel>, OrderedReliableStream) /*, Box<dyn error::Error>>*/
{
    ...
    let mut ClonedSocketSend = OtherSocket
        .try_clone()
        .expect("Unable to clone the TCP socket. :(");
    let (OtherSocketSendQueue_tx_c, WebRTCSendQueue_tx_c) =
        (OtherSocketSendQueue_tx.clone(), WebRTCSendQueue_tx.clone());
    ...
    let cu_udp_sq_to_udp = move || {
        ...
        loop {
            ...
            let recv_attempt = OtherSocketSendQueue_rx
...
fn write_offer_and_read_answer(
    ...
    if let Some(true) = config.Publish {
        if let Some(ref ptype) = config.PublishType {
            match (ptype.as_str()) {
                "ws" => {
                    return write_offer_and_read_answer_ws(local, config)
                        .unwrap()
                }
                _ => {
...
fn main() {
    ...
    if (config.WebRTCMode == "Offer") {
        ...
        let offerBase64TextTrimmed =
            write_offer_and_read_answer(local_description, config.clone());
        let offer =
        ...
        if (config.Type == "UDP") {
            ...
            //#[cfg(feature = "tcp")]
            {
                ...
                let BindPort =
                    config.Port.clone().expect("Binding port not specified");
