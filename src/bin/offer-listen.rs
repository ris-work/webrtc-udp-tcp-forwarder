...
async fn create_WebRTC_offer(config: &Config) -> Result<(Arc<RTCDataChannel>, Arc<RTCPeerConnection>, Option<RTCSessionDescription>), Box<dyn error::Error>> {
...
fn main() {
    ...
    if (config.WebRTCMode == "Offer") {
        ...
        let (mut data_channel, mut peer_connection, local_description) = rt.block_on(create_WebRTC_offer(&config)).expect("Failed creating a WebRTC Data Channel.");
        ...
        if (config.Type == "UDP") {
            ...
            #[cfg(feature = "tcp")]
            {
                ...
                let Listener = TcpListener::bind(format! {"{}:{}", BindAddress, BindPort}).expect(&format! {"Could not bind to TCP port: {}:{}", &BindAddress, &BindPort});
                info! {"Bound TCP on address {} port {}", BindAddress, BindPort};
                let mut OtherSocket = Listener.incoming().next().expect("Error getting the TCP stream").expect("TCP stream error");
